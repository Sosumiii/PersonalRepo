...
class swerveModule(commands2.Subsystem):
    def __init__(
        ...
        """ inverted = rotationConfig.inverted(True) """
        """ limits = rotationConfig.smartCurrentLimit(40) """
        """ #brake = rotationConfig.setIdleMode(rev.SparkBase.IdleMode.kBrake) """

        """ #rotationConfig.apply(brake) """
        """ rotationConfig.apply(limits) """
        """ rotationConfig.apply(inverted) """
        """ self.rotationMotor.configure(rotationConfig, rev.SparkBase.ResetMode.kNoResetSafeParameters, rev.SparkBase.PersistMode.kPersistParameters) """
